/******************************************************************************
 ...
 http://simple-circuit.com/                                                   *
                                                                              *
 This library had modify by TrungKuro                                         *
 I change something of protocol I2C, not use "stream"                         *
 When i make this, the address of Oled now is only use for 0x3C               *
                                                                              *
*******************************************************************************/
...
#define SSD1306_I2C_ADDRESS 0x3C  // Default original is 0x7A

#if !defined SSD1306_128_32 && !defined SSD1306_96_16
#define SSD1306_128_64            // Chossen display Oled 0.96inch
#endif
#if defined SSD1306_128_32 && defined SSD1306_96_16
...
void ssd1306_command(uint8_t c)
{
  ...
  i2c_start();            // I2C_Start(SSD1306_STREAM);
  i2c_write(0x78);        // I2C_Write(SSD1306_STREAM, _i2caddr);
  i2c_write(control);     // I2C_Write(SSD1306_STREAM, control);
  i2c_write(c);           // I2C_Write(SSD1306_STREAM, c);
  i2c_stop();             // I2C_Stop(SSD1306_STREAM);
}
...
void SSD1306_PutC(uint8_t c)
{
  ...
  i2c_start();          // I2C_Start(SSD1306_STREAM);
  i2c_write(0x78);      // I2C_Write(SSD1306_STREAM, _i2caddr);
  i2c_write(0x40);      // I2C_Write(SSD1306_STREAM, 0x40);
  ...
  for(uint8_t i = 0; i < 5; i++ )
  {
    ...
    i2c_write(font_c);  // I2C_Write(SSD1306_STREAM, font_c);
  }
  i2c_stop();           // I2C_Stop(SSD1306_STREAM);
...
void SSD1306_PutCustomC(char *c)
{
  ...
  {
    line = c[i];
    i2c_write(line);  // I2C_Write(SSD1306_STREAM, line);
  }
  i2c_stop();         // I2C_Stop(SSD1306_STREAM);
...
void SSD1306_ClearDisplay()
{
  ...
  for(uint16_t i = 0; i < SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8; i++ )
    i2c_write(0);   // I2C_Write(SSD1306_STREAM, 0);
...
void SSD1306_FillScreen()
{
  ...
  for(uint16_t i = 0; i < SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8; i++ )
    i2c_write(0xFF);  // I2C_Write(SSD1306_STREAM, 0xFF);
